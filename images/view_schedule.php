  ...
  <tr>
    ...
    <td><b>Action</b> </td>
		...
		//$strSQL1= "SELECT id, email, sub, CASE WHEN LENGTH(BODY) > 20 THEN CONCAT(SUBSTR(body,1,20), '...')  ELSE body END body , timezone.code, recur, next_run  FROM tasks, timezone WHERE  tasks.timezone = timezone.timezone AND user_id='$session_id';";
		 $strSQL1=  "SELECT id, email, sub, CASE WHEN LENGTH(BODY) > 20 THEN CONCAT(SUBSTR(body,1,20), '...') ELSE body END body , timezone.code, recur, next_run, DATE_FORMAT(next_run_on, '%a') as Week, DATE_FORMAT(next_run_on, '%m/%d/%y') DoW, DATE_FORMAT(next_run_on, '%b') month,  DATE_FORMAT(next_run_on, '%D') Day,  DATE_FORMAT(next_run_time, '%l:%i %p') Time, (next_run - NOW()) curr FROM tasks, timezone WHERE tasks.timezone = timezone.timezone AND user_id='$session_id' ORDER BY next_run DESC;";
		...
		if ($v_occur == 'Yearly') {	
		    ...
		    echo "<tr>";
        //echo "<td>". $v_email . "</td>";
        echo "<td>"; if ($v_current > 0) { echo "<img src='images/current.gif' class='none'>"; } else { echo "<img src='images/past.gif' class='none'>"; }  echo " ". $v_sub . "</td>"; 
        echo "<td>" . $v_body . "</td>"; 
         echo "<td>" . $v_occur ."</td>"; 
